...
class PasswordService {
  ...
  PasswordService(RequestExecutor requestExecutor) {
  ...
  private void providePassword(Map<String, String> parameters) {
    try {
      String htmlFromPasswordPageResponse = requestExecutor.executePostRequest(SSO_URL, SSO_URL, parameters);
      String samlUrl = passwordPageParser.extractSamlUrlFromPasswordPage(htmlFromPasswordPageResponse);

      requestExecutor.executeGetRequest(samlUrl, SSO_URL);
      requestExecutor.executeGetRequest(DISPATCHER_APP_URL, SSO_URL);
      requestExecutor.executeGetRequest(WELCOME_MESSAGE_URL, DISPATCHER_APP_URL);
      requestExecutor.executeGetRequest(REDIRECT_SSO_URL, DISPATCHER_APP_URL);
      requestExecutor.executeGetRequest(INDEX_URL, "");
    } catch (IOException e) {
